...
class AboutMessagePassing < Neo::Koan
  ...
  def test_methods_can_be_invoked_more_dynamically
    ...
    assert mc.send("caught?")
    assert mc.send("caught" + "?")     # добавляем ?
    assert mc.send("CAUGHT?".downcase)  # делаем маленькими буквами
  end
  ...
  def test_send_with_underscores_will_also_send_messages
    ...
    # THINK ABOUT IT:
    # Ruby использует `__send__`, чтобы избежать конфликтов с переопределенным методом `send`
  end
